...
for i, sample in enumerate(samples):
    ...
    cell_type_barcodes = {cell_type: cell_df[(cell_df['cell_type']== cell_type) & (cell_df[sample_value] == sample)]['cell_barcode'].to_list() for cell_type in cell_types}
    # Filter on the cell type barcodes
    cell_fragment = {cell_type: pl_fragment.filter(pl_fragment['name'].is_in(cell_type_barcodes[cell_type])) for cell_type in cell_types}
