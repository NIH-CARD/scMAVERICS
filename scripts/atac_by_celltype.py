...
all_samples = cell_data['SampleID'].drop_duplicates()
...
cell_data['sample_id'] = cell_data['SampleID']
cell_sample_batch = cell_data[['sample_id', 'batch']].drop_duplicates()
...
for sample, fragment_file in cell_type_fragments.items():
    cistopic_obj = create_cistopic_object_from_fragments(path_to_fragments=fragment_file,
                                               ...
                                               valid_bc = cell_data[cell_data['SampleID'] == sample]['barcode'].to_list(),
                                               n_cpu=1,
                                               project=sample
                                               )
    ...
    adata.obs = pd.merge(
        ...
        right=cell_data[cell_data['SampleID'] == sample][['cohort', 'Age', 'batch', 'PMI', 'atlas_identifier']],
...
adata.var['Annotation'] = adata.var['Annotation'].astype(str)
